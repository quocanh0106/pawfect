...
class VariantSelects extends HTMLElement {
  ...
  checkOption(option_name, option_value) {
    ...
    if(document.querySelector('#customily-cart-btn')) {
      document.querySelector('#customily-cart-btn').style.pointerEvents = 'auto!important';
    }
...
class ProductForm extends HTMLElement {
  ...
  async onSubmitHandler(evt) {
    ...
    const addOnProductId = this.form.querySelector('[name="add-on-product"]')?.value;
    const addOnConfig = fetchConfig('javascript');
    addOnConfig.headers['X-Requested-With'] = 'XMLHttpRequest';
    delete addOnConfig.headers['Content-Type'];

    if (addOnProductId && this.form.querySelector('[name="add-on-product"]').checked) {
      const addOnForm = new FormData();
      addOnForm.append('id', addOnProductId);
      addOnForm.append('quantity', 1)
      addOnConfig.body = addOnForm;
      await fetch(`${routes.cart_add_url}`, addOnConfig)
    }
